...
from forms.add_pack import add_package
...
def update_table():
    ...
    cursor.execute("SELECT id, name, price, x1, y1, x2, y2 FROM packages")
...
def get_data():
    ...
    with sqlite3.connect('db/dostavka.db') as db:
        ...
        cursor = db.cursor()
        query = """SELECT id, name, price, x1, y1, x2, y2 FROM packages"""
...
def open_window():
    if not hasattr(open_window, 'new_window') or not open_window.new_window.winfo_exists():
        ...
        open_window.new_window.title("Посылки")
...
def action_open(new_window):
    ...
    new_label = tk.Label(body, text="Список посылок",bg='white')
    new_label.pack()
